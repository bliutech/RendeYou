...
if (readenv.error) throw 'You need a .env file!';
...
app.put('/user/me', checkAuth, async (req, res) => {
  ...
  const updateEmail = updateOnlyIfChanged(
    'email',
    (email) => emailRegex.test(email),
    'Invalid email'
  );

  const updateFriends = updateOnlyIfChanged(
    'friends',
    async (friends) => {
      if (!friends.every(idRegex.test)) return false;
      const numFriends = await User.find()
        .where('_id')
        .in(friends)
        .countDocuments();
...
app.get('/event/:id([0-9a-f]{24})', async (req, res) => {
  ...
  if (event) {
    //make sure the event is valid, and if it is, send it
    res.send(event);
  } else {
...
app.delete('/event/:id([0-9a-f]{24})', checkAuth, async (req, res) => {
  ...
  if (event) {
    //if the event is valid, delete it
    Event.findByIdAndDelete(eventID, function (err) {
      //if there is some error in deleting it, then send a 403 error status
      if (err) {
        res.sendStatus(403);
      } else {
        res.sendStatus(200);
...
app.post(
  '/event/:id([0-9a-f]{24})/unsubscribe',
  checkAuth,
  async (req, res) => {
    const userId = req.session.userId;
    const event = await Event.findById(req.params.id);
